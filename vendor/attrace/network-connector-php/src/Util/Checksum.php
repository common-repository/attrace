...
class Checksum
{
    ...
    /**
     * Checksum CCITT based on byte array
     * @param $data
     * @return int CRC checksum
     */
    public static function checksumCCITT($data)
    {

        //   $crc = hexdec("0x0000");
        $crc = 0x0000;
        $crc = Crypto::byte16(~$crc);
        foreach ($data as $v) {
            $index = Crypto::byte8($crc) ^ $v;
     ...
     * @return array
     */
    public static function UInt16ToLittleEndianByteArray($checksum) {
        $crcArray = [];
        $crcArray[0] = Crypto::byte8($checksum);
        $crcArray[1] = Crypto::byte8($checksum >> 8);
        return $crcArray;
    }
}